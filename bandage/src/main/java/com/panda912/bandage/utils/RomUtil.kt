...
internal object RomUtil {
    ...
    @JvmStatic
    fun isSamsung() =
        Build.MANUFACTURER?.contains("samsung", true) ?: Build.BRAND?.contains("samsung", true)
        ?: false
    ...
    fun isGionee() = !getProp("ro.gn.sv.version").isNullOrEmpty()
            || (!TextUtils.isEmpty(Build.DISPLAY) && Build.DISPLAY.lowercase(Locale.getDefault())
        .contains("amigo"))
    ...
    @JvmStatic
    fun isQIKU() =
        Build.MANUFACTURER?.let { it.contains("QIKU", true) || it.contains("360") } ?: false
    ...
    private fun getProp(key: String): String? {
        try {
            BufferedReader(
                InputStreamReader(Runtime.getRuntime().exec("getprop $key").inputStream),
                1024
